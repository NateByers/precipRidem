...
watershed. In part 3 we will write functions and use loops to automate the data 
...
of a previous row. We'll use negation of this logical vector to subset out all
...
Another thing to notice is that there are negative values in the `LAB_RESULT` 
...
the `LAB_RESULT` column with the max threshold value (we just drop the `>`). 
For text values that are minimum thresholds (`<`) we'll replace the `numeric` 
value with _half_ of the minimum threshold
...
max.thresh.numeric <- gsub(">", "", max.thresh.df$LAB_RESULT_TEXT, fixed = T)

# convert the character vector to numeric
max.thresh.numeric <- as.numeric(max.thresh.numeric)
...
# create a character vector of LAB_RESULT_TEXT values with > removed
min.thresh.numeric <- gsub("<", "", min.thresh.df$LAB_RESULT_TEXT, fixed = T)

# convert the character vector to numeric
min.thresh.numeric <- as.numeric(min.thresh.numeric)
...
Also, this data set is in a "long" format. This 
means that there is one "value" column and one "variable" column (in this data 
set, the values are in the `LAB_RESULT` column and the variables are in the 
...
```{r}
colnames(wide.chem.df)[c(1:8, 33)] <- c("station", "activity", "date", "waterbody",
                                        "utm_e", "utm_n", "county", "value", 
                                        "turbidity_hach")
```
...
this, but first we need to make sure that the date columns for both data sets
